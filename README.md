...
Today the suite contains four effects:
...
- Four product implementations in `Source/vxsuite/products/`
...
All four current products support `Listen`.
...
This repository is the active standalone implementation home for the VX Suite line. The current code already reflects a clear shared framework and four distinct effect concepts, with the strongest validation currently centered on the de-reverb path.
