...
void VXRepairAudioProcessor::prepareSuite(double sampleRate, int samplesPerBlock) {
    ...
    // Pre-allocate scratch for advancing idle STFT state in listen mode.
    stftStateScratch.setSize(ch, std::max(1, samplesPerBlock), false, true, true);

    // Both STFT stages always run (at 0 strength when disabled) so the total
    // latency is constant.  Use noiseLat = max(denoiser, deepfilter) to cover
    // both noise paths.  Report it so the host compensates via PDC.
    setReportedLatencySamples(noiseLat + reverbLat);
}
...
void VXRepairAudioProcessor::processProduct(juce::AudioBuffer<float>& buffer,
    ...
    if (anyListen) {
        ...
        if (noiseListen) {
            ...
            juce::AudioBuffer<float> scratchView(stftStateScratch.getArrayOfWritePointers(), numCh, numSamples);
            denoiserDsp.processInPlace(scratchView, 0.0f, noiseOpts);
            juce::AudioBuffer<float> scratch2View(stftStateScratch.getArrayOfWritePointers(), numCh, numSamples);
    ...
    if (useDeepFilter && noiseOn) {
        ...
        const float noiseAmount = noiseOn ? noiseStr : 0.0f;
        const float preRms = noiseOn ? bufRms(buffer) : 0.0f;
        denoiserDsp.processInPlace(buffer, noiseAmount, noiseOpts);
        if (noiseOn) {
            const float wetRms = bufRms(buffer);
            const float gr = preRms > 1.0e-6f ? juce::jlimit(0.0f, 1.0f, 1.0f - wetRms / preRms) : 0.0f;
            noiseActivity.store(0.88f * noiseActivity.load(std::memory_order_relaxed) + 0.12f * gr, std::memory_order_relaxed);
    ...
    {
        const float reduce = reverbOn ? std::pow(reverbStr, 0.76f) : 0.0f;
        const float preRms = reverbOn ? bufRms(buffer) : 0.0f;
        deverbDsp.processInPlace(buffer, reduce, reverbOpts);
        if (reverbOn) {
            const float wetRms = bufRms(buffer);
