...
bool OnnxRuntime::loadModel(const juce::String& modelPath, juce::String& errorMessage) {
    ...
    // CoreML can be slower than CPU for this HS-TasNet ONNX graph because ORT only
    // partitions part of it. Keep it as an explicit diagnostic path until a
    // CoreML-native/static export proves faster.
    const auto coreMlMode = juce::SystemStats::getEnvironmentVariable("VXSTUDIO_REBALANCE_AI_ENABLE_COREML", "0")
        .trim()
        .toLowerCase();
    if (coreMlMode == "1" || coreMlMode == "true" || coreMlMode == "gpu") {
        const auto cacheDirectory = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
            .getChildFile("VX Suite")
            .getChildFile("RebalanceAI")
            .getChildFile("CoreMLCache");
        cacheDirectory.createDirectory();
        const auto cachePath = cacheDirectory.getFullPathName().toStdString();

        const char* coreMlKeys[] = {
            "ModelFormat",
            "MLComputeUnits",
        ...
        status = api->SessionOptionsAppendExecutionProvider(sessionOptions, "CoreML", coreMlKeys, coreMlValues, 5);
        if (status == nullptr) {
            gpuEnabled = true;
            gpuProviderName = "CoreML-CPUAndGPU";
            DBG("[ORT] CoreML execution provider enabled via MLProgram/CPUAndGPU/static-shape options");
        } else {
            DBG("[ORT] CoreML not available: " << api->GetErrorMessage(status) << " - falling back");
            api->ReleaseStatus(status);
            status = nullptr;
        }
    } else {
        DBG("[ORT] CoreML disabled by default; set VXSTUDIO_REBALANCE_AI_ENABLE_COREML=1 to test it");
    }
#endif

#if defined(_WIN32)
    // Windows priority: DirectML works across AMD, Intel and NVIDIA GPUs when using an ORT DirectML build.
    if (!gpuEnabled) {
        const char* dmlKeys[] = {"device_id"};
        const char* dmlValues[] = {"0"};
        constexpr const char* providerNames[] = {"DML", "DmlExecutionProvider"};
        for (const char* providerName : providerNames) {
            status = api->SessionOptionsAppendExecutionProvider(sessionOptions, providerName, dmlKeys, dmlValues, 1);
            if (status == nullptr) {
                gpuEnabled = true;
                gpuProviderName = "DirectML";
                DBG("[ORT] DirectML execution provider enabled");
                break;
            }
            DBG("[ORT] " << juce::String(providerName) << " not available: " << api->GetErrorMessage(status));
            api->ReleaseStatus(status);
            status = nullptr;
        }
    }
    ...
    if (status != nullptr) {
        const auto providerError = juce::String(api->GetErrorMessage(status));
        api->ReleaseStatus(status);
        status = nullptr;

        if (gpuEnabled) {
            DBG("[ORT] " << juce::String(gpuProviderName)
                         << " session creation failed: " << providerError
                         << " - retrying CPU");

            gpuEnabled = false;
            gpuProviderName = "CPU";

            OrtSessionOptions* cpuSessionOptions = nullptr;
            status = api->CreateSessionOptions(&cpuSessionOptions);
            if (status == nullptr && cpuSessionOptions != nullptr) {
                auto optionStatus = api->SetSessionGraphOptimizationLevel(cpuSessionOptions, ORT_ENABLE_ALL);
                if (optionStatus != nullptr)
                    api->ReleaseStatus(optionStatus);

                optionStatus = api->SetIntraOpNumThreads(cpuSessionOptions, numIntraOpThreads);
                if (optionStatus != nullptr)
                    api->ReleaseStatus(optionStatus);

                optionStatus = api->SetInterOpNumThreads(cpuSessionOptions, 1);
                if (optionStatus != nullptr)
                    api->ReleaseStatus(optionStatus);

#ifdef _WIN32
                status = api->CreateSession(ortEnv_.get(), wideModelPath.c_str(), cpuSessionOptions, &rawSession);
#else
                status = api->CreateSession(ortEnv_.get(), modelPath.toRawUTF8(), cpuSessionOptions, &rawSession);
#endif
                api->ReleaseSessionOptions(cpuSessionOptions);
            }
        }
