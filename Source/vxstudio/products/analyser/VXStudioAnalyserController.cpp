...
void AnalyserController::reconcileSelectionState(const std::vector<StageSnapshot>& inScope) noexcept {
    ...
    // If all remaining selected stages are inactive (stale/bypassed), fall back to full chain.
    if (!selectedInstanceIds.empty()) {
        std::unordered_set<std::uint64_t> active;
        for (const auto& s : inScope)
            if (!s.stale && !s.bypassed) active.insert(s.instanceId);
        bool anySelectedActive = false;
        for (const auto id : selectedInstanceIds)
            if (active.count(id)) { anySelectedActive = true; break; }
        if (!anySelectedActive)
            fullChainSelected = true;
    }
}
...
DryWetPair AnalyserController::resolveSelection(const std::vector<StageSnapshot>& inScope,
    ...
    if (!selectedInstanceIds.empty()) {
        ...
        // All selected became inactive — reconcileSelectionState already set fullChainSelected.
    }
...
void AnalyserController::refresh(const AnalyserContext& ctx,
    ...
    // 5. Reconcile selection state, then resolve dry/wet.
    reconcileSelectionState(filterResult.inScope);
