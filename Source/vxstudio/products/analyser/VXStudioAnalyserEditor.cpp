...
void VXStudioAnalyserEditor::paint(juce::Graphics& g) {
    ...
    auto plot = plotBounds.toFloat().reduced(60.0f, 28.0f);
    {
        ...
        for (int i = 0; i < vxsuite::analysis::kSummarySpectrumBins - 1; ++i) {
            ...
            const float x1 = xForFrequency(vxanalyser::AnalyserController::bandCenterHz(i),     plot);
            const float x2 = xForFrequency(vxanalyser::AnalyserController::bandCenterHz(i + 1), plot);
        ...
        const float markerX = xForFrequency(vxanalyser::AnalyserController::bandCenterHz(currentRenderModel.largestToneBand), plot);
        ...
        for (float hz : { 20.0f, 50.0f, 100.0f, 1000.0f, 10000.0f, 20000.0f }) {
            ...
            g.drawText(vxanalyser::AnalyserController::formatFrequency(hz),
...
juce::Path VXStudioAnalyserEditor::makeTonePath(
    ...
    for (int i = 0; i < n; ++i) {
        pts[static_cast<std::size_t>(i)] = {
            xForFrequency(vxanalyser::AnalyserController::bandCenterHz(i), bounds),
