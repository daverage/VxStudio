...
private:
    // A single row in the stage chain panel — either a track-group header or an effect row.
    struct ChainRow {
        ...
        std::uint64_t  trackStableId = 0;  // grouping key; same for all rows in a track group
        bool inactive      = false;
        bool selected      = false;
    ...
    struct RenderModel {
        ...
        bool analyserOnly = false;  // true when showing analyser's own input (no external stages)
    ...
    // A stage discovered in the shared StageRegistry, enriched with display metadata.
    struct StageEntry {
        vxsuite::analysis::StageView view;
        juce::String stageId;
        juce::String stageName;
        juce::String trackName;      // display label only — not used as identity key
        juce::Colour trackColour;
        std::uint64_t trackStableId = 0;   // grouping key derived from channelUID/runtimeID
        bool trackIsReliable = false;       // host confirmed the track identity
        juce::String stateText;
        juce::String impactText;
        juce::String typeLabel;
        juce::String freqHint;
    ...
    // Selection — all UI-thread only, no atomics needed.
    // instanceId is the primary key; index is never stored persistently.
    void selectStage(std::uint64_t instanceId, bool addToSelection);
    void selectTrack(std::uint64_t trackStableId);
    ...
    // --- Selection state (message thread only) ---
    // Ownership: runtimeInstanceId. Track grouping: trackStableId. Type: stageId only.
    std::unordered_set<std::uint64_t> selectedInstanceIds;
    bool fullChainSelectedValue = true;
