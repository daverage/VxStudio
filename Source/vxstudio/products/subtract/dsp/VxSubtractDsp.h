...
private:
    ...
    size_t frameBufferWritePos = 0;
    size_t olaReadPos  = 0;
    size_t olaWritePos = 0;

    // Stereo side-delay for M/S reconstruction
    std::vector<float> sideDelay;
    size_t sideDelayCap  = 0;
    size_t sideDelayRead = 0, sideDelayWrite = 0, sideDelayCount = 0;

    // Mid dry delay for latency-aligned reconstruction
    ...
    struct FrameCtx;
    void computeGainTargets(const FrameCtx& ctx, float energyRatio, bool learnFrameHasSignal);
    void smoothGains(bool subtractEnabled, float wetCore);
    void applyGainAndOLA();
    void reconstructStereo(juce::AudioBuffer<float>&, int processed, int n, float wet);
