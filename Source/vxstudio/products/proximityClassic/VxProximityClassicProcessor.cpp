#include "VxProximityClassicProcessor.h"
...
VXProximityClassicAudioProcessor::VXProximityClassicAudioProcessor()
    : ProcessorBase(makeIdentity()) {}

vxsuite::ProductIdentity VXProximityClassicAudioProcessor::makeIdentity() {
    ...
    identity.stageId          = "vx.ProximityClassic";
    identity.stageType        = vxsuite::StageType::mixed;
    identity.dspVersion       = vxsuite::versions::plugins::ProximityClassic;
...
juce::String VXProximityClassicAudioProcessor::getStatusText() const {
    if (isListenEnabled())
...
void VXProximityClassicAudioProcessor::prepareSuite(const double sampleRate,
...
void VXProximityClassicAudioProcessor::resetSuite() {
    proximityDsp.reset();
    outputTrimmer.reset();
...
void VXProximityClassicAudioProcessor::processProduct(juce::AudioBuffer<float>& buffer,
                                                juce::MidiBuffer&) {
...
void VXProximityClassicAudioProcessor::renderListenOutput(juce::AudioBuffer<float>& outputBuffer,
