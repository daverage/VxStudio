...
namespace vxsuite::ProximityClassic {
...
ProximityClassicDsp::BiquadCoeffs ProximityClassicDsp::makeLowShelf(const double sr, const float fcHz,
...
ProximityClassicDsp::BiquadCoeffs ProximityClassicDsp::makeHighShelf(const double sr, const float fcHz,
                                                         const float gainDb) noexcept {
    const float A    = std::pow(10.f, gainDb / 40.f);
    const float w0   = 2.f * juce::MathConstants<float>::pi * fcHz / static_cast<float>(sr);
    const float cosW = std::cos(w0);
...
void ProximityClassicDsp::applyBiquad(const BiquadCoeffs& c, BiquadState& s,
...
void ProximityClassicDsp::setChannelCount(const int numChannels) {
    chans.resize(static_cast<size_t>(std::max(1, numChannels)));
}

void ProximityClassicDsp::prepare(const double sampleRate, const int /*maxBlockSize*/) {
    sr = sampleRate > 1000.0 ? sampleRate : 48000.0;
    reset();
}

void ProximityClassicDsp::reset() noexcept {
    for (auto& ch : chans)
        ch = {};
    smoothedModel = {};
}
...
void ProximityClassicDsp::processInPlace(juce::AudioBuffer<float>& buffer,
