#include "VxStudioCorrectiveStage.h"
...
void CorrectiveStage::setParams(const SharedParams& newParams) {
    ...
    if (hiShelfConfigChanged) {
        if (newParams.hiShelfOn) {
            ...
            const float baseGainDb = voiceMode ? 2.12f : 4.8f;
            const float deEssScale = voiceMode ? 0.2f : 9.2f;
...
void CorrectiveStage::process(juce::AudioBuffer<float>& buffer) {
    ...
    const float troubleConfidence = juce::jlimit(0.0f, 1.0f, params.troubleConfidence);
    ...
    const float focusBias = juce::jlimit(0.0f, 1.0f, params.focusBias);
    const float persistentDensity = juce::jlimit(0.0f, 1.0f, params.persistentDensity);
    const float shortDensity = juce::jlimit(0.0f, 1.0f, params.shortDensity);
    const float densityPersistence = juce::jlimit(0.0f, 1.0f, params.densityPersistence);
    const float selfMaskLowMid = juce::jlimit(0.0f, 1.0f, params.selfMaskLowMid);
    const float selfMaskHigh = juce::jlimit(0.0f, 1.0f, params.selfMaskHigh);
    const float articulationRisk = juce::jlimit(0.0f, 1.0f, params.articulationRisk);
    const float bodyLossRisk = juce::jlimit(0.0f, 1.0f, params.bodyLossRisk);
    const float cumulativeRisk = juce::jlimit(0.0f, 1.0f, params.cumulativeRisk);
    const float tonalDriftRisk = juce::jlimit(0.0f, 1.0f, params.tonalDriftRisk);
    ...
    const float readabilityProtect = juce::jlimit(0.50f, 1.0f,
        1.0f - 0.48f * articulationRisk - 0.14f * cumulativeRisk + 0.10f * focusBias);
    const float bodyProtect = juce::jlimit(0.48f, 1.0f,
        1.0f - 0.44f * bodyLossRisk - 0.14f * selfMaskLowMid + 0.06f * (1.0f - focusBias));
    const float tonalProtect = juce::jlimit(0.46f, 1.0f,
        1.0f - 0.44f * tonalDriftRisk - 0.10f * selfMaskHigh);
    const float globalCorrectionGuard = juce::jlimit(0.42f, 1.0f,
        1.0f - 0.40f * cumulativeRisk - 0.14f * tonalDriftRisk);
    ...
    const float troubleDrive = troubleAmt * juce::jlimit(0.22f, 1.0f, 0.22f + 0.78f * troubleConfidence);
    const float smoothCtx = juce::jlimit(0.90f, 1.35f,
        1.00f + 0.30f * params.artifactRisk - 0.03f * speechPresence);
    const float troubleProtect = juce::jlimit(0.72f, 1.0f,
        (1.0f - (voiceMode ? 0.52f : 0.30f) * correctiveProtect)
      * readabilityProtect
      * tonalProtect
      * globalCorrectionGuard
      * juce::jlimit(0.78f, 1.0f, 0.90f - 0.18f * selfMaskHigh - 0.10f * cumulativeRisk));
    const float troubleRiskGate = juce::jlimit(0.40f, 1.0f,
        1.0f - (voiceMode ? 0.42f : 0.26f) * articulationRisk
              - 0.22f * tonalDriftRisk
              - 0.18f * cumulativeRisk
              - 0.12f * std::max(0.0f, deEssAmt - 0.55f * troubleConfidence)
              - 0.08f * std::max(0.0f, breathAmt - 0.65f * troubleConfidence));
    ...
    const float deMudThresholdDb = (voiceMode ? 1.5f : 2.5f) + 2.0f * loudNorm - 1.2f * proxCtx
        + 0.5f * bodyLossRisk + 0.25f * selfMaskLowMid - 0.18f * focusBias;
    const float deMudRangeDb = 10.0f;
    const float deMudProtect = juce::jlimit(0.68f, 1.0f,
        (1.0f - (voiceMode ? 0.22f : 0.12f) * correctiveProtect)
      * bodyProtect
      * globalCorrectionGuard
      * juce::jlimit(0.80f, 1.05f,
          0.84f + 0.12f * densityPersistence + 0.08f * persistentDensity - 0.05f * shortDensity));
    ...
    const float deEssProtect = juce::jlimit(0.66f, 1.0f,
        (1.0f - (voiceMode ? 0.62f : 0.32f) * correctiveProtect)
      * readabilityProtect
      * tonalProtect
      * juce::jlimit(0.78f, 1.0f, 0.90f - 0.18f * selfMaskHigh - 0.10f * shortDensity));
    ...
    const float breathProtect = juce::jlimit(0.64f, 1.0f,
        (1.0f - (voiceMode ? 0.58f : 0.28f) * correctiveProtect)
      * readabilityProtect
      * tonalProtect
      * juce::jlimit(0.74f, 1.0f, 0.88f - 0.18f * selfMaskHigh - 0.10f * articulationRisk));
    const float breathMaxCutDb = (voiceMode ? 7.0f : 2.8f) * breathAmt
        * (0.85f + 0.15f * (1.0f - speechPresence)) * breathProtect;
    const float plosiveProtect = juce::jlimit(0.58f, 1.0f,
    ...
    if (troubleDrive > 1.0e-6f) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            for (size_t b = 0; b < 6; ++b) {
                ...
                bandCutSum += troubleMaxCut[b] * troubleDrive * drive * troubleProtect * troubleRiskGate;
            }
    ...
    const float plosiveFocusShield = juce::jlimit(0.55f, 1.0f,
        1.0f - 0.38f * focusBias);
    const float plosiveMeterGuard = juce::jlimit(0.25f, 1.0f,
        plosiveFocusShield * (1.0f - (voiceMode ? 0.70f : 0.16f) * voicePreserve * speechPresence));
    plosiveActivity = juce::jlimit(0.0f, 1.0f,
        (plosiveAcc / static_cast<float>(numSamples)) * plosiveMeterGuard);
