...
float DenoiserDsp::wrapPi(float x) noexcept {
    return std::remainder(x, 2.0f * juce::MathConstants<float>::pi);
}

float DenoiserDsp::hzToBark(float hz) noexcept {
...
void DenoiserDsp::processFrame(const float amount,
    ...
    for (int k = 0; k < kBins; ++k) {
        ...
        tonalness[k] = clamp01((tonalRatio - 1.25f) / 2.8f);
    ...
    for (int k = 0; k < kBins; ++k) {
        ...
        if (k == 0 || k == kBins - 1) {
            ...
            if (phaseReady) {
                ...
                const float dphi = wrapPi((phIn - prevPhaseIn[k]) - phaseAdv[k]);
                phOut = wrapPi(prevPhaseOut[k] + phaseAdv[k] + dphi);
            }
